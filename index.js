...
//to delete the service providers in Admin panel
app.delete("/product/:id",async (req,resp)=>{
    const result = await ProviderInfo.deleteOne({_id:req.params.id})
    resp.send(result)
})

app.get("/product/:id",async (req,resp)=>{
    let result=await ProviderInfo.findOne({_id:req.params.id});
    if(result){
        resp.send(result)
    }else{
...
app.put("/product/:id",async (req,resp)=>{
    let result=await ProviderInfo.updateOne(
        {_id:req.params.id},
        {
            $set:req.body
        }
